...
public class SemanticChecker implements ASTVisitor {
    ...
    public void visit(classDefNode it){
    	curClass = (classType) globalScope.typeMap.get(it.name);
    ...
    public void visit(identifierExprNode it){
    	it.type = localScope.getVariableType(it.name, true, it.pos);
    }
    ...
    public void visit(creatorExprNode it){
		if (it.exprs != null) 
            it.exprs.forEach(x -> {x.accept(this); if (x.type.isInt == false) throw new semanticError("not int", x.pos);});
    }
    ...
    public void visit(memberExprNode it){
        ...
        if (it.base.type.isString == true && it.isFunc && it.name.equals("length")) {/*
            funcSymbol func = new funcSymbol("length");
            func.returnType = new primitiveType("int");*/
            it.type = new funcType("int");
            return;
        }
        if (it.base.type.isString == true && it.isFunc && it.name.equals("substring")) {/*
        ...
        if (it.base.type.isString == true && it.isFunc && it.name.equals("parseInt")) {/*
            funcSymbol func = new funcSymbol("parseInt");
            func.returnType = new primitiveType("int");*/
            it.type = new funcType("int");;
            return;
        }
        if (it.base.type.isString == true && it.isFunc && it.name.equals("ord")) {/*
    ...
    public void visit(funcExprNode it){
    	if (it.base instanceof identifierExprNode) 
            it.base.type = localScope.getFunctionType(((varExpr) it.base).name, true);
        else
        	it.base.accept(this);
        if (!(it.base.type instanceof funcType))
        	throw new semanticError("member isn't a function", it.pos);
        it.exprs.forEach(unit -> unit.accept(this));
        if (it.head.type.parameters.size() != it.parameters.size())
        	throw new semanticError("mismatched parameter's size", it.pos);
        for (int i = 0; i < it.head.type.parameters.size(); i++) {
            if (!it.head.type.parameters.get(i).equals(it.parameters.get(i).type))
    ...
    public void visit(subscriptExprNode it){
        ...
        if (it.offset.type.isInt == false)
        	throw new semanticError("index isn't int", it.pos);
        if (!(it.base.type instanceof arrayType))
        	throw new semanticError("subscript not array", it.pos);
        arrayType type = (arrayType) it.base.type;
        if (type.dim - 1 == 0)
        	it.type = type.type;
        else
    ...
    public void visit(forStatNode it) {
        ...
        if (it.condition != null && !it.condition.type.isBool)
            throw new semanticError("condition of for isn't bool", it.pos);
        if (it.inc != null)
        	it.inc.accept(this);
        loopDep++;
        localScope = new Scope(localScope);
    ...
    public void visit(ifStatNode it) {
        ...
        if (!it.condition.type.isBool)
        	throw new semanticError("condition of if isn't bool", it.pos);
        localScope = new Scope(localScope);
    ...
    public void visit(whileStatNode it) {
    	it.condition.accept(this);
        if (!it.condition.type.isBool())
        	throw new semanticError("condition of while isn't bool", it.pos);
        loopDep++;
        localScope = new Scope(localScope);
        it.body.accept(this);
        localScope = localScope.parentScope;
